!function() {
    ...
    function d3_map_size() {
        var size = 0;
        for (var key in this) key.charCodeAt(0) === d3_map_prefixCode && ++size;
        return size;
    }
    ...
    function d3_dispatch_event(dispatch) {
        function event() {
            var l, z = listeners, i = -1, n = z.length;
            while (n > ++i) (l = z[i].on) && l.apply(this, arguments);
            return dispatch;
        }
        var listeners = [], listenerByName = new d3_Map();
        event.on = function(name, listener) {
            var i, l = listenerByName.get(name);
            if (2 > arguments.length) return l && l.on;
            if (l) {
                l.on = null;
    ...
    function d3_eventDispatch(target) {
        ...
        while (n > ++i) dispatch[arguments[i]] = d3_dispatch_event(dispatch);
    ...
    function d3_selection_attr(name, value) {
        ...
        return null == value ? name.local ? attrNullNS : attrNull : "function" == typeof value ? name.local ? attrFunctionNS : attrFunction : name.local ? attrConstantNS : attrConstant;
    }
    ...
    function d3_selection_classed(name, value) {
        function classedConstant() {
            var i = -1;
            while (n > ++i) name[i](this, value);
        }
        function classedFunction() {
            var i = -1, x = value.apply(this, arguments);
            while (n > ++i) name[i](this, x);
        }
    ...
    function d3_selection_classedName(name) {
        ...
        return function(node, value) {
            ...
            if (value) {
                re.lastIndex = 0;
                re.test(c) || node.setAttribute("class", d3_collapse(c + " " + name));
    ...
    function d3_selection_sortComparator(comparator) {
        arguments.length || (comparator = d3.ascending);
        return function(a, b) {
    ...
    function d3_selection_on(type, listener, capture) {
        ...
        filter && (type = filter, wrap = d3_selection_onFilter);
    ...
    function d3_mousePoint(container, e) {
        ...
        if (svg.createSVGPoint) {
            ...
            d3_mouse_bug44083 ? (point.x = e.pageX, point.y = e.pageY) : (point.x = e.clientX, 
            point.y = e.clientY);
    ...
    function d3_hsl_rgb(h, s, l) {
        function v(h) {
            ...
            if (60 > h) return m1 + (m2 - m1) * h / 60;
            if (180 > h) return m2;
            if (240 > h) return m1 + (m2 - m1) * (240 - h) / 60;
            return m1;
        }
        function vv(h) {
    ...
    function d3_rgb_parse(format, rgb, hsl) {
        ...
        if (null != format && "#" === format.charAt(0)) {
            if (4 === format.length) {
                r = format.charAt(1);
                r += r;
                g = format.charAt(2);
    ...
    function d3_rgb_xyz(r) {
        return .04045 >= (r /= 255) ? r / 12.92 : Math.pow((r + .055) / 1.055, 2.4);
    }
    ...
    function d3_xhrType(response) {
        return function(url, mimeType, callback) {
            2 === arguments.length && "function" == typeof mimeType && (callback = mimeType, 
            mimeType = null);
    ...
    function d3_xhr(url, mimeType, response, callback) {
        ...
        !d3_window.XDomainRequest || "withCredentials" in request || !/^(http(s)?:)?\/\//.test(url) || (request = new XDomainRequest());
        ...
        xhr.header = function(name, value) {
            ...
            if (2 > arguments.length) return headers[name];
            null == value ? delete headers[name] : headers[name] = value + "";
            return xhr;
        };
        xhr.mimeType = function(value) {
            if (!arguments.length) return mimeType;
            mimeType = null == value ? null : value + "";
            return xhr;
        };
        ...
        xhr.send = function(method, data, callback) {
            2 === arguments.length && "function" == typeof data && (callback = data, data = null);
            request.open(method, url, true);
            null == mimeType || "accept" in headers || (headers["accept"] = mimeType + ",*/*");
            if (request.setRequestHeader) for (var name in headers) request.setRequestHeader(name, headers[name]);
            null != mimeType && request.overrideMimeType && request.overrideMimeType(mimeType);
        ...
        return null == callback ? xhr : xhr.get(d3_xhr_fixCallback(callback));
    }
    ...
    function d3_timer_sweep() {
        var t0, t1 = d3_timer_queueHead, time = 1/0;
        while (t1) if (t1.f) t1 = t0 ? t0.n = t1.n : d3_timer_queueHead = t1.n; else {
            time > t1.t && (time = t1.t);
            t1 = (t0 = t1).n;
        }
    ...
    function d3_locale_numberFormat(locale) {
        ...
        return function(specifier) {
            ...
            precision && (precision = +precision.substring(1));
            if (zfill || "0" === fill && "=" === align) {
                zfill = fill = "0";
                align = "=";
            ...
            "$" === symbol && (prefix = locale_currency[0], suffix = locale_currency[1]);
            ...
            return function(value) {
                ...
                var i = value.lastIndexOf("."), before = 0 > i ? value : value.substring(0, i), after = 0 > i ? "" : locale_decimal + value.substring(i + 1);
                !zfill && comma && (before = formatGroup(before));
                ...
                zcomma && (before = formatGroup(padding + before));
                negative += prefix;
                value = before + after;
    ...
    function d3_time_interval(local, step, number) {
        ...
        function range(t0, t1, dt) {
            ...
            if (dt > 1) while (t1 > time) {
                ...
                step(time, 1);
            } else while (t1 > time) times.push(new Date(+time)), step(time, 1);
            return times;
        }
    ...
    function d3_locale_timeFormat(locale) {
        function d3_time_format(template) {
            function format(date) {
                var c, p, f, string = [], i = -1, j = 0;
                while (n > ++i) if (37 === template.charCodeAt(i)) {
            ...
            format.parse = function(string) {
                ...
                if ("j" in d) date.setFullYear(d.y, 0, d.j); else if ("w" in d && ("W" in d || "U" in d)) {
                    ...
                    date.setFullYear(d.y, 0, "W" in d ? (d.w + 6) % 7 + 7 * d.W - (date.getDay() + 5) % 7 : d.w + 7 * d.U - (date.getDay() + 6) % 7);
        ...
        function d3_time_parse(date, template, string, j) {
            ...
            while (n > i) {
                ...
                if (37 === c) {
                    ...
                    if (!p || 0 > (j = p(date, string, j))) return -1;
    ...
    function d3_time_formatLookup(names) {
        ...
        while (n > ++i) map.set(names[i].toLowerCase(), i);
        return map;
    }
    ...
    function d3_time_formatMulti(formats) {
        ...
        while (n > ++i) formats[i][0] = this(formats[i][0]);
    ...
    function d3_geo_streamLine(coordinates, listener, closed) {
        ...
        listener.lineStart();
        while (n > ++i) coordinate = coordinates[i], listener.point(coordinate[0], coordinate[1], coordinate[2]);
        listener.lineEnd();
    }
    function d3_geo_streamPolygon(coordinates, listener) {
        var i = -1, n = coordinates.length;
        listener.polygonStart();
        while (n > ++i) d3_geo_streamLine(coordinates[i], listener, 1);
        listener.polygonEnd();
    }
    ...
    function d3_geo_sphericalEqual(a, b) {
        return ε > abs(a[0] - b[0]) && ε > abs(a[1] - b[1]);
    }
    ...
    function d3_geo_clipPolygon(segments, compare, clipStartInside, interpolate, listener) {
        ...
        var points, point, start = subject[0];
        while (1) {
        ...
        return function(rotate, listener) {
            ...
            function ringEnd() {
                ...
                if (1 & clean) {
                    ...
                    listener.lineStart();
                    while (n > ++i) listener.point((point = segment[i])[0], point[1]);
                    listener.lineEnd();
                    return;
                }
            ...
            var clip = {
                ...
                polygonEnd: function() {
                    ...
                    if (segments.length) d3_geo_clipPolygon(segments, d3_geo_clipSort, clipStartInside, interpolate, listener); else if (clipStartInside) {
                        listener.lineStart();
            ...
            var segments;
            var polygon, ring, buffer = d3_geo_clipBufferListener(), ringListener = clipLine(buffer);
            return clip;
        };
    }
    ...
    function d3_geo_clipSort(a, b) {
        return (0 > (a = a.x)[0] ? a[1] - halfπ - ε : halfπ - a[1]) - (0 > (b = b.x)[0] ? b[1] - halfπ - ε : halfπ - b[1]);
    }
        ...
        return {
            ...
            point: function(λ1, φ1) {
                ...
                if (ε > abs(dλ - π)) {
                    ...
                    ε > abs(λ0 - sλ0) && (λ0 -= sλ0 * ε);
                    ε > abs(λ1 - sλ1) && (λ1 -= sλ1 * ε);
    ...
    function d3_geo_clipCircle(radius) {
        ...
        function clipLine(listener) {
            ...
            return {
                ...
                point: function(λ, φ) {
                    ...
                    !point0 && (v00 = v0 = v) && listener.lineStart();
                    if (v !== v0) {
        ...
        function intersect(a, b, two) {
            ...
            var δλ = λ1 - λ0, polar = ε > abs(δλ - π), meridian = polar || ε > δλ;
            !polar && φ0 > φ1 && (z = φ0, φ0 = φ1, φ1 = z);
            if (meridian ? polar ? φ0 + φ1 > 0 ^ q[1] < (ε > abs(q[0] - λ0) ? φ0 : φ1) : q[1] >= φ0 && φ1 >= q[1] : δλ > π ^ (q[0] >= λ0 && λ1 >= q[0])) {
    ...
    function d3_geo_clipExtent(x0, y0, x1, y1) {
        function corner(p, direction) {
            return ε > abs(p[0] - x0) ? direction > 0 ? 0 : 3 : ε > abs(p[0] - x1) ? direction > 0 ? 2 : 1 : ε > abs(p[1] - y0) ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
        }
        ...
        return function(listener) {
            function insidePolygon(p) {
                ...
                for (var i = 0; n > i; ++i) for (var b, j = 1, v = polygon[i], m = v.length, a = v[0]; m > j; ++j) {
                    b = v[j];
                    y >= a[1] ? b[1] > y && d3_cross2d(a, b, p) > 0 && ++wn : y >= b[1] && 0 > d3_cross2d(a, b, p) && --wn;
                    a = b;
                }
                return 0 !== wn;
            }
            function interpolate(from, to, direction, listener) {
                var a = 0, a1 = 0;
                if (null == from || (a = corner(from, direction)) !== (a1 = corner(to, direction)) || 0 > comparePoints(from, to) ^ direction > 0) do listener.point(0 === a || 3 === a ? x0 : x1, a > 1 ? y1 : y0); while ((a = (a + direction + 4) % 4) !== a1); else listener.point(to[0], to[1]);
            }
            ...
            function linePoint(x, y) {
                ...
                } else if (v && v_) listener.point(x, y); else {
                    var l = {
                        a: {
                            x: x_,
                            y: y_
            ...
            var segments, polygon, ring, listener_ = listener, bufferListener = d3_geo_clipBufferListener(), clipLine = d3_geom_clipLine(x0, y0, x1, y1);
            var clip = {
                point: point,
                lineStart: lineStart,
                lineEnd: lineEnd,
    ...
    function d3_geo_conic(projectAt) {
        ...
        p.parallels = function(_) {
            if (!arguments.length) return [ 180 * (φ0 / π), 180 * (φ1 / π) ];
    ...
    function d3_geo_resample(project) {
        ...
        function resampleLineTo(x0, y0, λ0, a0, b0, c0, x1, y1, λ1, a1, b1, c1, depth, stream) {
            ...
            if (d2 > 4 * δ2 && depth--) {
                var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = Math.sqrt(a * a + b * b + c * c), φ2 = Math.asin(c /= m), λ2 = ε > abs(abs(c) - 1) || ε > abs(λ0 - λ1) ? (λ0 + λ1) / 2 : Math.atan2(b, a), p = project(λ2, φ2), x2 = p[0], y2 = p[1], dx2 = x2 - x0, dy2 = y2 - y0, dz = dy * dx2 - dx * dy2;
                if (dz * dz / d2 > δ2 || abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 || cosMinDistance > a0 * a1 + b0 * b1 + c0 * c1) {
    ...
    function d3_geo_projectionMutator(projectAt) {
        ...
        function invalidate() {
            stream && (stream.valid = false, stream = null);
            return projection;
        }
        ...
        projection.clipAngle = function(_) {
            ...
            preclip = null == _ ? (clipAngle = _, d3_geo_clipAntimeridian) : d3_geo_clipCircle((clipAngle = +_) * d3_radians);
            return invalidate();
        };
    ...
    function d3_geo_circleInterpolate(radius, precision) {
        ...
        return function(from, to, direction, listener) {
            ...
            if (null != from) {
                ...
                (direction > 0 ? to > from : from > to) && (from += direction * τ);
            } else {
    ...
    function d3_geo_circleAngle(cr, point) {
        ...
        return ((0 > -a[2] ? -angle : angle) + 2 * Math.PI - ε) % (2 * Math.PI);
    }
    ...
    function d3_geo_conicConformal(φ0, φ1) {
        function forward(λ, φ) {
            var ρ = ε > abs(abs(φ) - halfπ) ? 0 : F / Math.pow(t(φ), n);
    ...
    function d3_geo_conicEquidistant(φ0, φ1) {
        ...
        if (ε > abs(n)) return d3_geo_equirectangular;
        forward.invert = function(x, y) {
            var ρ0_y = G - y;
    ...
    function d3_geom_hullUpper(points) {
        ...
        for (var i = 2; n > i; i++) {
            while (hs > 1 && 0 >= d3_cross2d(points[hull[hs - 2]], points[hull[hs - 1]], points[i])) --hs;
            hull[hs++] = i;
        }
    ...
    function d3_geom_voronoiRemoveBeach(beach) {
        ...
        var lArc = previous;
        while (lArc.circle && ε > abs(x - lArc.circle.x) && ε > abs(y - lArc.circle.cy)) {
            previous = lArc.P;
        ...
        var rArc = next;
        while (rArc.circle && ε > abs(x - rArc.circle.x) && ε > abs(y - rArc.circle.cy)) {
            next = rArc.N;
    ...
    function d3_geom_voronoiAddBeach(site) {
        var lArc, rArc, dxl, dxr, x = site.x, directrix = site.y, node = d3_geom_voronoiBeaches._;
        while (node) {
    ...
    function d3_geom_voronoiCloseCells(extent) {
        var x2, y2, x3, y3, cell, iHalfEdge, halfEdges, nHalfEdges, start, end, x0 = extent[0][0], x1 = extent[1][0], y0 = extent[0][1], y1 = extent[1][1], cells = d3_geom_voronoiCells, iCell = cells.length;
        while (iCell--) {
            cell = cells[iCell];
            ...
            iHalfEdge = 0;
            while (nHalfEdges > iHalfEdge) {
                end = halfEdges[iHalfEdge].end(), x3 = end.x, y3 = end.y;
                start = halfEdges[++iHalfEdge % nHalfEdges].start(), x2 = start.x, y2 = start.y;
                if (abs(x3 - x2) > ε || abs(y3 - y2) > ε) {
                    halfEdges.splice(iHalfEdge, 0, new d3_geom_voronoiHalfEdge(d3_geom_voronoiCreateBorderEdge(cell.site, end, ε > abs(x3 - x0) && y1 - y3 > ε ? {
                        x: x0,
                        y: ε > abs(x2 - x0) ? y2 : y1
                    } : ε > abs(y3 - y1) && x1 - x3 > ε ? {
                        x: ε > abs(y2 - y1) ? x2 : x1,
    ...
    function d3_geom_voronoiAttachCircle(arc) {
        ...
        while (node) if (circle.y < node.y || circle.y === node.y && circle.x <= node.x) {
            if (!node.L) {
                before = node.P;
                break;
            }
    ...
    function d3_geom_voronoiDetachCircle(arc) {
        ...
        if (circle) {
            circle.P || (d3_geom_voronoiFirstCircle = circle.N);
    ...
    function d3_geom_voronoiClipEdges(extent) {
        var e, edges = d3_geom_voronoiEdges, clip = d3_geom_clipLine(extent[0][0], extent[0][1], extent[1][0], extent[1][1]), i = edges.length;
        while (i--) {
            e = edges[i];
            if (!d3_geom_voronoiConnectEdge(e, extent) || !clip(e) || ε > abs(e.a.x - e.b.x) && ε > abs(e.a.y - e.b.y)) {
                e.a = e.b = null;
                edges.splice(i, 1);
            }
        }
    ...
    function d3_geom_voronoiSetEdgeEnd(edge, lSite, rSite, vertex) {
        if (edge.a || edge.b) edge.l === rSite ? edge.b = vertex : edge.a = vertex; else {
            edge.a = vertex;
            edge.l = lSite;
            edge.r = rSite;
        }
    ...
    function d3_interpolateString(a, b) {
        ...
        b.length > s0 && s.push(b.substring(s0));
        for (i = 0, n = q.length; (m = d3_interpolate_number.exec(a)) && n > i; ++i) {
            o = q[i];
            if (o.x == m[0]) {
                if (o.i) if (null == s[o.i + 1]) {
                    s[o.i - 1] += o.x;
                    s.splice(o.i, 1);
                    for (j = i + 1; n > j; ++j) q[j].i--;
                } else {
                    s[o.i - 1] += o.x + s[o.i + 1];
                    s.splice(o.i, 2);
                    for (j = i + 1; n > j; ++j) q[j].i -= 2;
                } else if (null == s[o.i + 1]) s[o.i] = o.x; else {
                    s[o.i] = o.x + s[o.i + 1];
                    s.splice(o.i + 1, 1);
                    for (j = i + 1; n > j; ++j) q[j].i--;
                }
        ...
        while (n > i) {
            o = q.pop();
            if (null == s[o.i + 1]) s[o.i] = o.x; else {
                s[o.i] = o.x + s[o.i + 1];
                s.splice(o.i + 1, 1);
            }
            n--;
        }
        if (1 === s.length) return null == s[0] ? (o = q[0].x, function(t) {
            return o(t) + "";
        }) : function() {
            return b;
        };
    ...
    function d3_interpolate(a, b) {
        var f, i = d3.interpolators.length;
    ...
    function d3_ease_elastic(a, p) {
        var s;
        2 > arguments.length && (p = .45);
        arguments.length ? s = p / τ * Math.asin(1 / a) : (a = 1, s = p / 4);
        return function(t) {
    ...
    function d3_interpolateTransform(a, b) {
        ...
        } else tb[0] || tb[1] ? s.push("translate(" + tb + ")") : s.push("");
        if (ra != rb) {
            ra - rb > 180 ? rb += 360 : rb - ra > 180 && (ra += 360);
            q.push({
                i: s.push(s.pop() + "rotate(", null, ")") - 2,
                x: d3_interpolateNumber(ra, rb)
            });
        } else rb && s.push(s.pop() + "rotate(" + rb + ")");
        wa != wb ? q.push({
        ...
        } else (1 != kb[0] || 1 != kb[1]) && s.push(s.pop() + "scale(" + kb + ")");
        n = q.length;
        return function(t) {
            var o, i = -1;
            while (n > ++i) s[(o = q[i]).i] = o.x(t);
            return s.join("");
        };
    }
    ...
    function d3_layout_forceAccumulate(quad, alpha, charges) {
        ...
        quad.charge = 0;
        if (!quad.leaf) {
            var c, nodes = quad.nodes, n = nodes.length, i = -1;
            while (n > ++i) {
                c = nodes[i];
                if (null == c) continue;
    ...
    function d3_layout_stackMaxIndex(array) {
        var k, i = 1, j = 0, v = array[0][1], n = array.length;
        for (;n > i; ++i) if ((k = array[i][1]) > v) {
            j = i;
            v = k;
        }
        return j;
    ...
    function d3_layout_treeSearch(node, compare) {
        ...
        if (children && (n = children.length)) {
            var child, n, i = -1;
            while (n > ++i) compare(child = d3_layout_treeSearch(children[i], compare), node) > 0 && (node = child);
        }
        return node;
    }
    ...
    function d3_layout_treeVisitAfter(node, callback) {
        function visit(node, previousSibling) {
            var children = node.children;
            if (children && (n = children.length)) {
                var child, n, previousChild = null, i = -1;
                while (n > ++i) {
                    child = children[i];
    ...
    function d3_layout_treeShift(node) {
        var child, shift = 0, change = 0, children = node.children, i = children.length;
        while (--i >= 0) {
        ...
        if (n > 1) {
            ...
            if (n > 2) {
                ...
                for (i = 3; n > i; i++) {
                    ...
                    if (1 == isect) for (k = a._pack_prev; k !== j._pack_prev; k = k._pack_prev, s2++) if (d3_layout_packIntersects(k, c)) break;
                    if (isect) {
                        s2 > s1 || s1 == s2 && b.r < a.r ? d3_layout_packSplice(a, b = j) : d3_layout_packSplice(a = k, b);
                        i--;
                    } else {
                        d3_layout_packInsert(a, c);
                        b = c;
    ...
    function d3_layout_packTransform(node, x, y, k) {
        ...
        if (children) {
            ...
            while (n > ++i) d3_layout_packTransform(children[i], x, y, k);
        }
    }
    ...
    function d3_scale_linearTickRange(domain, m) {
        ...
        .15 >= err ? step *= 10 : .35 >= err ? step *= 5 : .75 >= err && (step *= 2);
    ...
    function d3_scale_log(linear, base, positive, domain) {
        ...
        scale.ticks = function() {
            ...
            if (isFinite(j - i)) {
                if (positive) {
                    ...
                    ticks.push(pow(i));
                    for (;j > i++; ) for (var k = n - 1; k > 0; k--) ticks.push(pow(i) * k);
                }
                for (i = 0; u > ticks[i]; i++) ;
        ...
        scale.tickFormat = function(n, format) {
            ...
            2 > arguments.length ? format = d3_scale_logFormat : "function" != typeof format && (format = d3.format(format));
            var e, k = Math.max(.1, n / scale.ticks().length), f = positive ? (e = 1e-12, Math.ceil) : (e = -1e-12, 
            Math.floor);
            return function(d) {
                return k >= d / pow(f(log(d) + e)) ? format(d) : "";
            };
        };
        scale.copy = function() {
    ...
    function d3_scale_ordinal(domain, ranger) {
        ...
        scale.domain = function(x) {
            ...
            domain = [];
            index = new d3_Map();
            var xi, i = -1, n = x.length;
            while (n > ++i) index.has(xi = x[i]) || index.set(xi, domain.push(xi));
        ...
        scale.rangePoints = function(x, padding) {
            2 > arguments.length && (padding = 0);
            var start = x[0], stop = x[1], step = (stop - start) / (Math.max(1, domain.length - 1) + padding);
            range = steps(2 > domain.length ? (start + stop) / 2 : start + step * padding / 2, step);
            rangeBand = 0;
            ranger = {
                t: "rangePoints",
                a: arguments
        ...
        scale.rangeBands = function(x, padding, outerPadding) {
            2 > arguments.length && (padding = 0);
            3 > arguments.length && (outerPadding = padding);
    ...
    function d3_scale_quantile(domain, range) {
        function rescale() {
            ...
            thresholds = [];
            while (q > ++k) thresholds[k - 1] = d3.quantile(domain, k / q);
            return scale;
        }
        function scale(x) {
        ...
        scale.invertExtent = function(y) {
            y = range.indexOf(y);
            return 0 > y ? [ 0/0, 0/0 ] : [ y > 0 ? thresholds[y - 1] : domain[0], thresholds.length > y ? thresholds[y] : domain[domain.length - 1] ];
        };
        scale.copy = function() {
    ...
    function d3_svg_line(projection) {
        function line(data) {
            ...
            var d, segments = [], points = [], i = -1, n = data.length, fx = d3_functor(x), fy = d3_functor(y);
            while (n > ++i) if (defined.call(this, d = data[i], i)) points.push([ +fx.call(this, d, i), +fy.call(this, d, i) ]); else if (points.length) {
                segment();
                points = [];
            }
            points.length && segment();
        ...
        line.interpolate = function(_) {
            ...
            interpolateKey = "function" == typeof _ ? interpolate = _ : (interpolate = d3_svg_lineInterpolators.get(_) || d3_svg_lineLinear).key;
            return line;
        };
    ...
    function d3_svg_lineStep(points) {
        ...
        while (n > ++i) path.push("H", (p[0] + (p = points[i])[0]) / 2, "V", p[1]);
        n > 1 && path.push("H", p[0]);
        return path.join("");
    }
    function d3_svg_lineStepBefore(points) {
        var i = 0, n = points.length, p = points[0], path = [ p[0], ",", p[1] ];
        while (n > ++i) path.push("V", (p = points[i])[1], "H", p[0]);
        return path.join("");
    }
    function d3_svg_lineStepAfter(points) {
        var i = 0, n = points.length, p = points[0], path = [ p[0], ",", p[1] ];
        while (n > ++i) path.push("H", (p = points[i])[0], "V", p[1]);
        return path.join("");
    }
    function d3_svg_lineCardinalOpen(points, tension) {
        return 4 > points.length ? d3_svg_lineLinear(points) : points[1] + d3_svg_lineHermite(points.slice(1, points.length - 1), d3_svg_lineCardinalTangents(points, tension));
    }
    function d3_svg_lineCardinalClosed(points, tension) {
        return 3 > points.length ? d3_svg_lineLinear(points) : points[0] + d3_svg_lineHermite((points.push(points[0]), 
        points), d3_svg_lineCardinalTangents([ points[points.length - 2] ].concat(points, [ points[1] ]), tension));
    }
    function d3_svg_lineCardinal(points, tension) {
        return 3 > points.length ? d3_svg_lineLinear(points) : points[0] + d3_svg_lineHermite(points, d3_svg_lineCardinalTangents(points, tension));
    }
    function d3_svg_lineHermite(points, tangents) {
        if (1 > tangents.length || points.length != tangents.length && points.length != tangents.length + 2) return d3_svg_lineLinear(points);
        var quad = points.length != tangents.length, path = "", p0 = points[0], p = points[1], t0 = tangents[0], t = t0, pi = 1;
        if (quad) {
            path += "Q" + (p[0] - 2 * t0[0] / 3) + "," + (p[1] - 2 * t0[1] / 3) + "," + p[0] + "," + p[1];
            p0 = points[1];
            pi = 2;
        }
        if (tangents.length > 1) {
            ...
            for (var i = 2; tangents.length > i; i++, pi++) {
                p = points[pi];
                t = tangents[i];
    ...
    function d3_svg_lineCardinalTangents(points, tension) {
        var p0, tangents = [], a = (1 - tension) / 2, p1 = points[0], p2 = points[1], i = 1, n = points.length;
        while (n > ++i) {
            p0 = p1;
            p1 = p2;
            p2 = points[i];
    ...
    function d3_svg_lineBasis(points) {
        if (3 > points.length) return d3_svg_lineLinear(points);
    ...
    function d3_svg_lineBasisOpen(points) {
        if (4 > points.length) return d3_svg_lineLinear(points);
        var pi, path = [], i = -1, n = points.length, px = [ 0 ], py = [ 0 ];
        while (3 > ++i) {
            pi = points[i];
            px.push(pi[0]);
            py.push(pi[1]);
    ...
    function d3_svg_lineBasisClosed(points) {
        var path, pi, i = -1, n = points.length, m = n + 4, px = [], py = [];
        while (4 > ++i) {
            pi = points[i % n];
            px.push(pi[0]);
            py.push(pi[1]);
    ...
    function d3_svg_lineBundle(points, tension) {
        ...
        if (n) {
            var p, t, x0 = points[0][0], y0 = points[0][1], dx = points[n][0] - x0, dy = points[n][1] - y0, i = -1;
            while (n >= ++i) {
                p = points[i];
                t = i / n;
    ...
    function d3_svg_lineFiniteDifferences(points) {
        ...
        while (j > ++i) m[i] = (d + (d = d3_svg_lineSlope(p0 = p1, p1 = points[i + 1]))) / 2;
        m[i] = d;
        return m;
    }
    function d3_svg_lineMonotoneTangents(points) {
        var d, a, b, s, tangents = [], m = d3_svg_lineFiniteDifferences(points), i = -1, j = points.length - 1;
        while (j > ++i) {
            d = d3_svg_lineSlope(points[i], points[i + 1]);
            if (ε > abs(d)) m[i] = m[i + 1] = 0; else {
                a = m[i] / d;
                b = m[i + 1] / d;
                s = a * a + b * b;
                if (s > 9) {
    ...
    function d3_svg_lineMonotone(points) {
        return 3 > points.length ? d3_svg_lineLinear(points) : points[0] + d3_svg_lineHermite(points, d3_svg_lineMonotoneTangents(points));
    }
    function d3_svg_lineRadial(points) {
        var point, r, a, i = -1, n = points.length;
        while (n > ++i) {
            point = points[i];
            r = point[0];
    ...
    function d3_svg_area(projection) {
        function area(data) {
            ...
            var d, x, y, segments = [], points0 = [], points1 = [], i = -1, n = data.length, fx0 = d3_functor(x0), fy0 = d3_functor(y0), fx1 = x0 === x1 ? function() {
                return x;
            } : d3_functor(x1), fy1 = y0 === y1 ? function() {
                return y;
            } : d3_functor(y1);
            while (n > ++i) if (defined.call(this, d = data[i], i)) {
    ...
    function d3_transitionNode(node, i, id, inherit) {
        ...
        if (!transition) {
            ...
            d3.timer(function(elapsed) {
                ...
                function stop() {
                    --lock.count ? delete lock[id] : delete node.__transition__;
                    return 1;
                }
                var d = node.__data__, ease = transition.ease, delay = transition.delay, duration = transition.duration, timer = d3_timer_active, tweened = [];
                timer.t = delay + time;
                if (elapsed >= delay) return start(elapsed - delay);
                timer.c = start;
            }, 0, time);
        }
    }
    ...
    function d3_time_scale(linear, methods, format) {
        ...
        scale.nice = function(interval, skip) {
            ...
            method && (interval = method[0], skip = method[1]);
        ...
        scale.ticks = function(interval, skip) {
            ...
            method && (interval = method[0], skip = method[1]);
            return interval.range(extent[0], d3_time_scaleDate(+extent[1] + 1), 1 > skip ? 1 : skip);
        };
    ...
    d3.min = function(array, f) {
        ...
        if (1 === arguments.length) {
            while (n > ++i && !(null != (a = array[i]) && a >= a)) a = void 0;
            while (n > ++i) null != (b = array[i]) && a > b && (a = b);
        } else {
            while (n > ++i && !(null != (a = f.call(array, array[i], i)) && a >= a)) a = void 0;
            while (n > ++i) null != (b = f.call(array, array[i], i)) && a > b && (a = b);
        }
        return a;
    };
    d3.max = function(array, f) {
        ...
        if (1 === arguments.length) {
            while (n > ++i && !(null != (a = array[i]) && a >= a)) a = void 0;
            while (n > ++i) null != (b = array[i]) && b > a && (a = b);
        } else {
            while (n > ++i && !(null != (a = f.call(array, array[i], i)) && a >= a)) a = void 0;
            while (n > ++i) null != (b = f.call(array, array[i], i)) && b > a && (a = b);
        }
        return a;
    };
    d3.extent = function(array, f) {
        ...
        if (1 === arguments.length) {
            while (n > ++i && !(null != (a = c = array[i]) && a >= a)) a = c = void 0;
            while (n > ++i) if (null != (b = array[i])) {
                a > b && (a = b);
                b > c && (c = b);
            }
        } else {
            while (n > ++i && !(null != (a = c = f.call(array, array[i], i)) && a >= a)) a = void 0;
            while (n > ++i) if (null != (b = f.call(array, array[i], i))) {
                a > b && (a = b);
                b > c && (c = b);
            }
        }
    ...
    d3.sum = function(array, f) {
        ...
        if (1 === arguments.length) while (n > ++i) isNaN(a = +array[i]) || (s += a); else while (n > ++i) isNaN(a = +f.call(array, array[i], i)) || (s += a);
        return s;
    };
    d3.mean = function(array, f) {
        var a, n = array.length, m = 0, i = -1, j = 0;
        if (1 === arguments.length) while (n > ++i) d3_number(a = array[i]) && (m += (a - m) / ++j); else while (n > ++i) d3_number(a = f.call(array, array[i], i)) && (m += (a - m) / ++j);
        return j ? m : void 0;
    };
    ...
    d3.median = function(array, f) {
        arguments.length > 1 && (array = array.map(f));
        array = array.filter(d3_number);
        return array.length ? d3.quantile(array.sort(d3.ascending), .5) : void 0;
    };
    d3.bisector = function(f) {
        return {
            left: function(a, x, lo, hi) {
                3 > arguments.length && (lo = 0);
                4 > arguments.length && (hi = a.length);
                while (hi > lo) {
                    var mid = lo + hi >>> 1;
                    x > f.call(a, a[mid], mid) ? lo = mid + 1 : hi = mid;
                }
                return lo;
            },
            right: function(a, x, lo, hi) {
                ...
                while (hi > lo) {
                    var mid = lo + hi >>> 1;
                    f.call(a, a[mid], mid) > x ? hi = mid : lo = mid + 1;
                }
                return lo;
            }
        };
    ...
    d3.pairs = function(array) {
        var p0, i = 0, n = array.length - 1, p1 = array[0], pairs = new Array(0 > n ? 0 : n);
        while (n > i) pairs[i] = [ p0 = p1, p1 = array[++i] ];
        return pairs;
    };
    d3.zip = function() {
        if (!(n = arguments.length)) return [];
        for (var i = -1, m = d3.min(arguments, d3_zipLength), zips = new Array(m); m > ++i; ) for (var n, j = -1, zip = zips[i] = new Array(n); n > ++j; ) zip[j] = arguments[j][i];
        return zips;
    };
    ...
    d3.merge = function(arrays) {
        var m, merged, array, n = arrays.length, i = -1, j = 0;
        while (n > ++i) j += arrays[i].length;
        merged = new Array(j);
        while (--n >= 0) {
            array = arrays[n];
            m = array.length;
    ...
    d3.range = function(start, stop, step) {
        ...
        if (1/0 === (stop - start) / step) throw new Error("infinite range");
        var j, range = [], k = d3_range_integerScale(abs(step)), i = -1;
        start *= k, stop *= k, step *= k;
        if (0 > step) while ((j = start + step * ++i) > stop) range.push(j / k); else while (stop > (j = start + step * ++i)) range.push(j / k);
        return range;
    };
    ...
    d3.nest = function() {
        function map(mapType, array, depth) {
            ...
            var keyValue, object, setter, values, i = -1, n = array.length, key = keys[depth++], valuesByKey = new d3_Map();
            while (n > ++i) (values = valuesByKey.get(keyValue = key(object = array[i]))) ? values.push(object) : valuesByKey.set(keyValue, [ object ]);
            if (mapType) {
                object = mapType();
    ...
    d3.rebind = function(target, source) {
        var method, i = 1, n = arguments.length;
        while (n > ++i) target[method = arguments[i]] = d3_rebind(target, source, source[method]);
        return target;
    };
    ...
    d3_dispatch.prototype.on = function(type, listener) {
        ...
        if (type) return 2 > arguments.length ? this[type].on(name) : this[type].on(name, listener);
        if (2 === arguments.length) {
    ...
    d3_selectionPrototype.select = function(selector) {
        ...
        for (var j = -1, m = this.length; m > ++j; ) {
            subgroups.push(subgroup = []);
            subgroup.parentNode = (group = this[j]).parentNode;
            for (var i = -1, n = group.length; n > ++i; ) if (node = group[i]) {
                subgroup.push(subnode = selector.call(node, node.__data__, i, j));
                subnode && "__data__" in node && (subnode.__data__ = node.__data__);
    ...
    d3_selectionPrototype.selectAll = function(selector) {
        ...
        for (var j = -1, m = this.length; m > ++j; ) for (var group = this[j], i = -1, n = group.length; n > ++i; ) if (node = group[i]) {
        ...
        if (2 > arguments.length) {
            if ("string" == typeof name) {
                ...
                if (value = node.classList) {
                    while (n > ++i) if (!value.contains(name[i])) return false;
                } else {
                    value = node.getAttribute("class");
                    while (n > ++i) if (!d3_selection_classedRe(name[i]).test(value)) return false;
                }
                return true;
            }
    ...
    d3_selectionPrototype.data = function(value, key) {
        function bind(group, groupData) {
            ...
            if (key) {
                ...
                for (i = -1; m > ++i; ) {
                    ...
                    } else dataByKeyValue.has(keyValue) || (enterNodes[i] = d3_selection_dataNode(nodeData));
                    dataByKeyValue.set(keyValue, nodeData);
                    nodeByKeyValue.remove(keyValue);
                }
                for (i = -1; n > ++i; ) nodeByKeyValue.has(keyValues[i]) && (exitNodes[i] = group[i]);
            } else {
                for (i = -1; n0 > ++i; ) {
                    node = group[i];
        ...
        if (!arguments.length) {
            ...
            while (n > ++i) (node = group[i]) && (value[i] = node.__data__);
            return value;
        }
        var enter = d3_selection_enter([]), update = d3_selection([]), exit = d3_selection([]);
        if ("function" == typeof value) while (n > ++i) bind(group = this[i], value.call(group, group.parentNode.__data__, i)); else while (n > ++i) bind(group = this[i], value);
    ...
    d3_selectionPrototype.order = function() {
        for (var j = -1, m = this.length; m > ++j; ) for (var node, group = this[j], i = group.length - 1, next = group[i]; --i >= 0; ) if (node = group[i]) {
            next && next !== node.nextSibling && next.parentNode.insertBefore(node, next);
            next = node;
        }
        return this;
    };
    d3_selectionPrototype.sort = function(comparator) {
        ...
        for (var j = -1, m = this.length; m > ++j; ) this[j].sort(comparator);
        return this.order();
    };
    ...
    d3_selection_enterPrototype.select = function(selector) {
        var subgroup, subnode, upgroup, group, node, subgroups = [];
        for (var j = -1, m = this.length; m > ++j; ) {
    ...
    d3_selection_enterPrototype.insert = function(name, before) {
        2 > arguments.length && (before = d3_selection_enterInsertBefore(this));
        return d3_selectionPrototype.insert.call(this, name, before);
    };
    d3_selectionPrototype.transition = function() {
        var subgroup, node, id = d3_transitionInheritId || ++d3_transitionId, subgroups = [], transition = d3_transitionInherit || {
            time: Date.now(),
        ...
        for (var j = -1, m = this.length; m > ++j; ) {
            subgroups.push(subgroup = []);
            for (var group = this[j], i = -1, n = group.length; n > ++i; ) {
    ...
    d3.touches = function(container, touches) {
        2 > arguments.length && (touches = d3_eventSource().touches);
    ...
    d3.behavior.zoom = function() {
        ...
        function touchstarted() {
            ...
            function started() {
                ...
                for (var i = 0, n = changed.length; n > i; ++i) locations0[changed[i].identifier] = null;
            ...
            function moved() {
                var p0, l0, p1, l1, touches = d3.touches(target);
            ...
            function ended() {
                if (d3.event.touches.length) {
                    var changed = d3.event.changedTouches;
                    for (var i = 0, n = changed.length; n > i; ++i) delete locations0[changed[i].identifier];
        ...
        zoom.scaleExtent = function(_) {
            ...
            scaleExtent = null == _ ? d3_behavior_zoomInfinity : [ +_[0], +_[1] ];
            return zoom;
        };
    ...
    var d3_behavior_zoomInfinity = [ 0, 1/0 ];
    ...
    d3.dsv = function(delimiter, mimeType) {
        function dsv(url, row, callback) {
            3 > arguments.length && (callback = row, row = null);
        ...
        dsv.parseRows = function(text, f) {
            function token() {
                ...
                if (34 === text.charCodeAt(j)) {
                    var i = j;
                    while (N > i++) if (34 === text.charCodeAt(i)) {
                    ...
                    if (13 === c) {
                        eol = true;
                        10 === text.charCodeAt(i + 2) && ++I;
                    } else 10 === c && (eol = true);
                    return text.substring(j + 1, i).replace(/""/g, '"');
                }
                while (N > I) {
                    var c = text.charCodeAt(I++), k = 1;
                    if (10 === c) eol = true; else if (13 === c) {
                        eol = true;
            ...
            var t, eol, EOL = {}, EOF = {}, rows = [], N = text.length, I = 0, n = 0;
            while ((t = token()) !== EOF) {
                var a = [];
                while (t !== EOL && t !== EOF) {
                    a.push(t);
    ...
    var d3_geo_streamObjectType = {
        ...
        FeatureCollection: function(object, listener) {
            ...
            while (n > ++i) d3_geo_streamGeometry(features[i].geometry, listener);
        }
    };
    var d3_geo_streamGeometryType = {
        ...
        MultiPoint: function(object, listener) {
            ...
            while (n > ++i) object = coordinates[i], listener.point(object[0], object[1], object[2]);
        },
        ...
        MultiLineString: function(object, listener) {
            var coordinates = object.coordinates, i = -1, n = coordinates.length;
            while (n > ++i) d3_geo_streamLine(coordinates[i], listener, 0);
        },
        ...
        MultiPolygon: function(object, listener) {
            var coordinates = object.coordinates, i = -1, n = coordinates.length;
            while (n > ++i) d3_geo_streamPolygon(coordinates[i], listener);
        },
        GeometryCollection: function(object, listener) {
            var geometries = object.geometries, i = -1, n = geometries.length;
            while (n > ++i) d3_geo_streamGeometry(geometries[i], listener);
        }
    };
    ...
    d3.geo.bounds = function() {
        ...
        function linePoint(λ, φ) {
            ...
            if (p0) {
                ...
                if (antimeridian) λ_ > λ ? angle(λ0, λ) > angle(λ0, λ1) && (λ1 = λ) : angle(λ, λ1) > angle(λ0, λ1) && (λ0 = λ); else if (λ1 >= λ0) {
                    λ0 > λ && (λ0 = λ);
                    λ > λ1 && (λ1 = λ);
                } else λ > λ_ ? angle(λ0, λ) > angle(λ0, λ1) && (λ1 = λ) : angle(λ, λ1) > angle(λ0, λ1) && (λ0 = λ);
            } else point(λ, φ);
            p0 = p, λ_ = λ;
        }
        function lineStart() {
        ...
        function angle(λ0, λ1) {
            return 0 > (λ1 -= λ0) ? λ1 + 360 : λ1;
        }
        function compareRanges(a, b) {
            return a[0] - b[0];
        }
        function withinRange(x, range) {
            return range[0] <= range[1] ? x >= range[0] && range[1] >= x : range[0] > x || x > range[1];
        }
        ...
        return function(feature) {
            ...
            if (n) {
                ...
                var dλ, best = -1/0;
                for (var b, n = merged.length - 1, i = 0, a = merged[n]; n >= i; a = b, ++i) {
                    b = merged[i];
                    (dλ = angle(a[1], b[0])) > best && (best = dλ, λ0 = b[0], λ1 = a[1]);
                }
            }
            ranges = range = null;
            return 1/0 === λ0 || 1/0 === φ0 ? [ [ 0/0, 0/0 ], [ 0/0, 0/0 ] ] : [ [ λ0, φ0 ], [ λ1, φ1 ] ];
        };
    }();
    ...
    d3.geo.albersUsa = function() {
        ...
        var point, lower48Point, alaskaPoint, hawaiiPoint, pointStream = {
            point: function(x, y) {
                point = [ x, y ];
            }
        };
    ...
    d3.geo.path = function() {
        function path(object) {
            if (object) {
                "function" == typeof pointRadius && contextStream.pointRadius(+pointRadius.apply(this, arguments));
        ...
        path.centroid = function(object) {
            ...
            return d3_geo_centroidZ2 ? [ d3_geo_centroidX2 / d3_geo_centroidZ2, d3_geo_centroidY2 / d3_geo_centroidZ2 ] : d3_geo_centroidZ1 ? [ d3_geo_centroidX1 / d3_geo_centroidZ1, d3_geo_centroidY1 / d3_geo_centroidZ1 ] : d3_geo_centroidZ0 ? [ d3_geo_centroidX0 / d3_geo_centroidZ0, d3_geo_centroidY0 / d3_geo_centroidZ0 ] : [ 0/0, 0/0 ];
        };
        path.bounds = function(object) {
            d3_geo_pathBoundsX1 = d3_geo_pathBoundsY1 = -(d3_geo_pathBoundsX0 = d3_geo_pathBoundsY0 = 1/0);
        ...
        path.context = function(_) {
            ...
            contextStream = null == (context = _) ? new d3_geo_pathBuffer() : new d3_geo_pathContext(_);
    ...
    d3.geo.greatArc = function() {
        ...
        greatArc.source = function(_) {
            ...
            source = _, source_ = "function" == typeof _ ? null : _;
            return greatArc;
        };
        greatArc.target = function(_) {
            if (!arguments.length) return target;
            target = _, target_ = "function" == typeof _ ? null : _;
            return greatArc;
        };
    ...
    d3.geom.hull = function(vertices) {
        function hull(data) {
            ...
            for (i = 0; n > i; i++) flippedPoints.push([ points[i][0], -points[i][1] ]);
            var upper = d3_geom_hullUpper(points), lower = d3_geom_hullUpper(flippedPoints);
            var skipLeft = lower[0] === upper[0], skipRight = lower[lower.length - 1] === upper[upper.length - 1], polygon = [];
            for (i = upper.length - 1; i >= 0; --i) polygon.push(data[points[upper[i]][2]]);
            for (i = +skipLeft; lower.length - skipRight > i; ++i) polygon.push(data[points[lower[i]][2]]);
            return polygon;
        }
    ...
    d3_geom_polygonPrototype.area = function() {
        var a, i = -1, n = this.length, b = this[n - 1], area = 0;
        while (n > ++i) {
            a = b;
            b = this[i];
    ...
    d3_geom_polygonPrototype.centroid = function(k) {
        ...
        arguments.length || (k = -1 / (6 * this.area()));
        while (n > ++i) {
            a = b;
            b = this[i];
            c = a[0] * b[1] - b[0] * a[1];
    ...
    d3_geom_polygonPrototype.clip = function(subject) {
        var input, j, m, b, c, d, closed = d3_geom_polygonClosed(subject), i = -1, n = this.length - d3_geom_polygonClosed(this), a = this[n - 1];
        while (n > ++i) {
            input = subject.slice();
            subject.length = 0;
            b = this[i];
            ...
            while (m > ++j) {
                ...
                if (d3_geom_polygonInside(d, a, b)) {
                    d3_geom_polygonInside(c, a, b) || subject.push(d3_geom_polygonIntersect(c, d, a, b));
                    subject.push(d);
                } else d3_geom_polygonInside(c, a, b) && subject.push(d3_geom_polygonIntersect(c, d, a, b));
                c = d;
            }
    ...
    d3_geom_voronoiCell.prototype.prepare = function() {
        var edge, halfEdges = this.edges, iHalfEdge = halfEdges.length;
        while (iHalfEdge--) {
    ...
    d3_geom_voronoiRedBlackTree.prototype = {
        ...
        remove: function(node) {
            ...
            next = left ? right ? d3_geom_voronoiRedBlackFirst(right) : left : right;
    ...
    d3.geom.voronoi = function(points) {
        function voronoi(data) {
            ...
            d3_geom_voronoi(sites(data), clipExtent).cells.forEach(function(cell, i) {
                var edges = cell.edges, site = cell.site, polygon = polygons[i] = edges.length ? edges.map(function(e) {
                    var s = e.start();
                    return [ s.x, s.y ];
                }) : site.x >= x0 && x1 >= site.x && site.y >= y0 && y1 >= site.y ? [ [ x0, y1 ], [ x1, y1 ], [ x1, y0 ], [ x0, y0 ] ] : [];
        ...
        voronoi.triangles = function(data) {
            ...
            d3_geom_voronoi(sites(data)).cells.forEach(function(cell, i) {
                var e0, s0, site = cell.site, edges = cell.edges.sort(d3_geom_voronoiHalfEdgeOrder), j = -1, m = edges.length, e1 = edges[m - 1].edge, s1 = e1.l === site ? e1.r : e1.l;
                while (m > ++j) {
                    e0 = e1;
                    s0 = s1;
                    e1 = edges[j].edge;
                    s1 = e1.l === site ? e1.r : e1.l;
                    s0.i > i && s1.i > i && 0 > d3_geom_voronoiTriangleArea(site, s0, s1) && triangles.push([ data[i], data[s0.i], data[s1.i] ]);
                }
            });
            return triangles;
        };
        ...
        voronoi.clipExtent = function(_) {
            ...
            clipExtent = null == _ ? d3_geom_voronoiClipExtent : _;
            return voronoi;
        };
    ...
    d3.geom.quadtree = function(points, x1, y1, x2, y2) {
        function quadtree(data) {
            function insert(n, d, x, y, x1, y1, x2, y2) {
                ...
                if (n.leaf) {
                    var nx = n.x, ny = n.y;
                    if (null != nx) if (.01 > abs(nx - x) + abs(ny - y)) insertChild(n, d, x, y, x1, y1, x2, y2); else {
                        var nPoint = n.point;
            ...
            if (null != x1) x1_ = x1, y1_ = y1, x2_ = x2, y2_ = y2; else {
                x2_ = y2_ = -(x1_ = y1_ = 1/0);
                xs = [], ys = [];
                n = data.length;
                if (compat) for (i = 0; n > i; ++i) {
                    d = data[i];
                    x1_ > d.x && (x1_ = d.x);
                    y1_ > d.y && (y1_ = d.y);
                    d.x > x2_ && (x2_ = d.x);
            ...
            i = -1;
            if (null == x1) {
                while (n > ++i) insert(root, data[i], xs[i], ys[i], x1_, y1_, x2_, y2_);
                --i;
        ...
        quadtree.extent = function(_) {
            if (!arguments.length) return null == x1 ? null : [ [ x1, y1 ], [ x2, y2 ] ];
            null == _ ? x1 = y1 = x2 = y2 = null : (x1 = +_[0][0], y1 = +_[0][1], x2 = +_[1][0], 
            y2 = +_[1][1]);
            return quadtree;
        };
        quadtree.size = function(_) {
            if (!arguments.length) return null == x1 ? null : [ x2 - x1, y2 - y1 ];
            null == _ ? x1 = y1 = x2 = y2 = null : (x1 = y1 = 0, x2 = +_[0], y2 = +_[1]);
            return quadtree;
        };
        return quadtree;
    };
    ...
    d3.layout.bundle = function() {
        return function(links) {
            ...
            while (n > ++i) paths.push(d3_layout_bundlePath(links[i]));
            return paths;
        };
    };
    ...
    d3.layout.force = function() {
        ...
        force.tick = function() {
            ...
            if (charge) {
                ...
                i = -1;
                while (n > ++i) (o = nodes[i]).fixed || q.visit(repulse(o));
            }
            i = -1;
            while (n > ++i) {
                o = nodes[i];
        ...
        force.linkDistance = function(x) {
            ...
            linkDistance = "function" == typeof x ? x : +x;
            return force;
        };
        force.distance = force.linkDistance;
        force.linkStrength = function(x) {
            if (!arguments.length) return linkStrength;
            linkStrength = "function" == typeof x ? x : +x;
            return force;
        };
        ...
        force.alpha = function(x) {
            ...
            x = +x;
            if (alpha) alpha = x > 0 ? x : 0; else if (x > 0) {
                event.start({
                    type: "start",
                    alpha: alpha = x
                });
        ...
        force.start = function() {
            function position(dimension, size) {
                ...
                var x, candidates = neighbors[i], j = -1, m = candidates.length;
                while (m > ++j) if (!isNaN(x = candidates[j][dimension])) return x;
            ...
            distances = [];
            if ("function" == typeof linkDistance) for (i = 0; m > i; ++i) distances[i] = +linkDistance.call(this, links[i], i); else for (i = 0; m > i; ++i) distances[i] = linkDistance;
            strengths = [];
            if ("function" == typeof linkStrength) for (i = 0; m > i; ++i) strengths[i] = +linkStrength.call(this, links[i], i); else for (i = 0; m > i; ++i) strengths[i] = linkStrength;
            charges = [];
            if ("function" == typeof charge) for (i = 0; n > i; ++i) charges[i] = +charge.call(this, nodes[i], i); else for (i = 0; n > i; ++i) charges[i] = charge;
            return force.resume();
        };
        ...
        force.drag = function() {
            drag || (drag = d3.behavior.drag().origin(d3_identity).on("dragstart.force", d3_layout_forceDragstart).on("drag.force", dragmove).on("dragend.force", d3_layout_forceDragend));
    ...
    d3.layout.hierarchy = function() {
        function recurse(node, depth, nodes) {
            ...
            if (childs && (n = childs.length)) {
                var n, d, i = -1, c = node.children = new Array(n), v = 0, j = depth + 1;
                while (n > ++i) {
        ...
        function revalue(node, depth) {
            ...
            if (children && (n = children.length)) {
                var n, i = -1, j = depth + 1;
                while (n > ++i) v += revalue(children[i], j);
    ...
    d3.layout.partition = function() {
        ...
        function depth(node) {
            ...
            if (children && (n = children.length)) {
                var n, i = -1;
                while (n > ++i) d = Math.max(d, depth(children[i]));
            }
            return 1 + d;
        }
    ...
    d3.layout.pie = function() {
        function pie(data) {
            ...
            var a = +("function" == typeof startAngle ? startAngle.apply(this, arguments) : startAngle);
            var k = (("function" == typeof endAngle ? endAngle.apply(this, arguments) : endAngle) - a) / d3.sum(values);
    ...
    d3.layout.stack = function() {
        function stack(data, index) {
            ...
            var i, j, o, n = series.length, m = series[0].length;
            for (j = 0; m > j; ++j) {
                out.call(stack, series[0][j], o = offsets[j], points[0][j][1]);
                for (i = 1; n > i; ++i) out.call(stack, series[i][j], o += points[i - 1][j][1], points[i][j][1]);
            }
            return data;
        }
        ...
        stack.offset = function(x) {
            ...
            offset = "function" == typeof x ? x : d3_layout_stackOffsets.get(x) || d3_layout_stackOffsetZero;
            return stack;
        };
        stack.x = function(z) {
            if (!arguments.length) return x;
    ...
    d3.layout.histogram = function() {
        function histogram(data, i) {
            var bin, x, bins = [], values = data.map(valuer, this), range = ranger.call(this, values, i), thresholds = binner.call(this, range, values, i), i = -1, n = values.length, m = thresholds.length - 1, k = frequency ? 1 : 1 / n;
            while (m > ++i) {
                bin = bins[i] = [];
        ...
        histogram.bins = function(x) {
            ...
            binner = "number" == typeof x ? function(range) {
    ...
    d3.layout.tree = function() {
        function tree(d, i) {
            function firstWalk(node, previousSibling) {
                ...
                if (children && (n = children.length)) {
                    var n, previousChild, child, firstChild = children[0], ancestor = firstChild, i = -1;
                    while (n > ++i) {
                        child = children[i];
            ...
            function secondWalk(node, x) {
                ...
                if (children && (n = children.length)) {
                    var n, i = -1;
                    x += node._tree.mod;
                    while (n > ++i) secondWalk(children[i], x);
                }
            }
            function apportion(node, previousSibling, ancestor) {
                if (previousSibling) {
                    var shift, vip = node, vop = node, vim = previousSibling, vom = node.parent.children[0], sip = vip._tree.mod, sop = vop._tree.mod, sim = vim._tree.mod, som = vom._tree.mod;
    ...
    d3.layout.pack = function() {
        ...
        pack.radius = function(_) {
            ...
            radius = null == _ || "function" == typeof _ ? _ : +_;
            return pack;
        };
    ...
    d3.layout.treemap = function() {
        function scale(children, k) {
            var child, area, i = -1, n = children.length;
            while (n > ++i) {
        ...
        function squarify(node) {
            ...
            if (children && children.length) {
                ...
                while ((n = remaining.length) > 0) {
                    ...
                    row.area += child.area;
                    if ("squarify" !== mode || best >= (score = worst(row, u))) {
                        remaining.pop();
                        best = score;
                    } else {
        ...
        function worst(row, u) {
            var r, s = row.area, rmax = 0, rmin = 1/0, i = -1, n = row.length;
            while (n > ++i) {
            ...
            s *= s;
            u *= u;
            return s ? Math.max(u * rmax * ratio / s, s / (u * rmin * ratio)) : 1/0;
        }
        function position(row, u, rect, flush) {
            var o, i = -1, n = row.length, x = rect.x, y = rect.y, v = u ? round(row.area / u) : 0;
            if (u == rect.dx) {
                (flush || v > rect.dy) && (v = rect.dy);
                while (n > ++i) {
                    o = row[i];
                    o.x = x;
                    o.y = y;
        ...
        var stickies, hierarchy = d3.layout.hierarchy(), round = Math.round, size = [ 1, 1 ], padding = null, pad = d3_layout_treemapPadNull, sticky = false, mode = "squarify", ratio = .5 * (1 + Math.sqrt(5));
        ...
        treemap.padding = function(x) {
            ...
            if (!arguments.length) return padding;
            var type;
            pad = null == (padding = x) ? d3_layout_treemapPadNull : "function" == (type = typeof x) ? padFunction : "number" === type ? (x = [ x, x, x, x ], 
    ...
    var d3_transitionInheritId, d3_transitionInherit, d3_transitionPrototype = [], d3_transitionId = 0;
    ...
    d3_transitionPrototype.select = function(selector) {
        ...
        for (var j = -1, m = this.length; m > ++j; ) {
            subgroups.push(subgroup = []);
            for (var group = this[j], i = -1, n = group.length; n > ++i; ) if ((node = group[i]) && (subnode = selector.call(node, node.__data__, i, j))) {
    ...
    d3_transitionPrototype.selectAll = function(selector) {
        ...
        for (var j = -1, m = this.length; m > ++j; ) for (var group = this[j], i = -1, n = group.length; n > ++i; ) if (node = group[i]) {
            ...
            subgroups.push(subgroup = []);
            for (var k = -1, o = subnodes.length; o > ++k; ) {
    ...
    d3_transitionPrototype.tween = function(name, tween) {
        var id = this.id;
        if (2 > arguments.length) return this.node().__transition__[id].tween.get(name);
        return d3_selection_each(this, null == tween ? function(node) {
    ...
    d3_transitionPrototype.remove = function() {
        return this.each("end.transition", function() {
            var p;
            2 > this.__transition__.count && (p = this.parentNode) && p.removeChild(this);
        });
    };
    d3_transitionPrototype.ease = function(value) {
        var id = this.id;
        if (1 > arguments.length) return this.node().__transition__[id].ease;
        "function" != typeof value && (value = d3.ease.apply(d3, arguments));
    ...
    d3_transitionPrototype.transition = function() {
        var subgroup, group, node, transition, id0 = this.id, id1 = ++d3_transitionId, subgroups = [];
        for (var j = 0, m = this.length; m > j; j++) {
            subgroups.push(subgroup = []);
    ...
    d3.svg.axis = function() {
        function axis(g) {
            g.each(function() {
                ...
                var tickTransform, ticks = null == tickValues ? scale1.ticks ? scale1.ticks.apply(scale1, tickArguments_) : scale1.domain() : tickValues, tickFormat = null == tickFormat_ ? scale1.tickFormat ? scale1.tickFormat.apply(scale1, tickArguments_) : d3_identity : tickFormat_, tick = g.selectAll(".tick").data(ticks, scale1), tickEnter = tick.enter().insert("g", ".domain").attr("class", "tick").style("opacity", ε), tickExit = d3.transition(tick.exit()).style("opacity", ε).remove(), tickUpdate = d3.transition(tick).style("opacity", 1);
                ...
                } else scale0.rangeBand ? scale0 = scale1 : tickExit.call(tickTransform, scale1);
    ...
    d3.svg.brush = function() {
        ...
        function brushstart() {
            ...
            function brushmove() {
                ...
                if (!dragging) if (d3.event.altKey) {
                    center || (center = [ (xExtent[0] + xExtent[1]) / 2, (yExtent[0] + yExtent[1]) / 2 ]);
            ...
            function move1(point, scale, i) {
                ...
                if (dragging) max = (min += position) + size; else {
                    center && (position = Math.max(r0, Math.min(r1, 2 * center[i] - min)));
                    if (min > position) {
                        max = min;
                        min = position;
                    } else max = position;
                ...
                if (extent[0] != min || extent[1] != max) {
                    i ? yExtentDomain = null : xExtentDomain = null;
                    extent[0] = min;
                    extent[1] = max;
                    return true;
                }
            ...
            var center, offset, target = this, eventTarget = d3.select(d3.event.target), event_ = event.of(target, arguments), g = d3.select(target), resizing = eventTarget.datum(), resizingX = !/^(n|s)$/.test(resizing) && x, resizingY = !/^(e|w)$/.test(resizing) && y, dragging = eventTarget.classed("extent"), dragRestore = d3_event_dragSuppress(), origin = d3.mouse(target);
            var w = d3.select(d3_window).on("keydown.brush", keydown).on("keyup.brush", keyup);
            d3.event.changedTouches ? w.on("touchmove.brush", brushmove).on("touchend.brush", brushend) : w.on("mousemove.brush", brushmove).on("mouseup.brush", brushend);
        ...
        var xExtentDomain, yExtentDomain, event = d3_eventDispatch(brush, "brushstart", "brush", "brushend"), x = null, y = null, xExtent = [ 0, 0 ], yExtent = [ 0, 0 ], xClamp = true, yClamp = true, resizes = d3_svg_brushResizes[0];
        ...
        brush.clamp = function(z) {
            ...
            x && y ? (xClamp = !!z[0], yClamp = !!z[1]) : x ? xClamp = !!z : y && (yClamp = !!z);
            return brush;
        };
        brush.extent = function(z) {
            var x0, x1, y0, y1, t;
            if (!arguments.length) {
                if (x) if (xExtentDomain) x0 = xExtentDomain[0], x1 = xExtentDomain[1]; else {
                    x0 = xExtent[0], x1 = xExtent[1];
                    x.invert && (x0 = x.invert(x0), x1 = x.invert(x1));
                    x0 > x1 && (t = x0, x0 = x1, x1 = t);
                }
                if (y) if (yExtentDomain) y0 = yExtentDomain[0], y1 = yExtentDomain[1]; else {
                    y0 = yExtent[0], y1 = yExtent[1];
                    y.invert && (y0 = y.invert(y0), y1 = y.invert(y1));
